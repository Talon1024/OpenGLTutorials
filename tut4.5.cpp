...
// Tutorial 4: Textures
// Exercise 4: Custom blend factor as shader uniform
// https://learnopengl.com/Getting-started/Textures

void framebuffer_size_callback(GLFWwindow* window, int width, int height);
void error_callback(int error, const char* description);
void processInput(GLFWwindow *window, float &blendFactor);
...
int main(int argc, char** argv) {
    ...
    {
        KShaderProgram theShader("tut4.vp", "tut4.5.fp");
        // Binds the "brickWall" uniform to texture unit 0.
        // All sampler2D uniforms are bound to texture unit 0 by default, so the following line is unnecessary.
        // theShader.setUniform("brickWall", 0);
        float blendFactor = 0.2; // This can be incremented or decremented using the arrow keys
        // Render loop - do not quit until I quit
        while (!glfwWindowShouldClose(window))
        {
            // Handle input
            processInput(window, blendFactor);
            ...
            // Use shader program
            theShader.use();
            // Bind the "awesomeFace" uniform to texture unit 1.
            // All sampler2D uniforms use texture unit 0 by default.
            theShader.setUniform("awesomeFace", 1);
            // Set the blendFactor value
